...
const answer = arr.reduce((acc, item) => {
  ...
  if (valid) {
    const newArr = acc.split(' ') || [];
    newArr.forEach(item2 => {
      if (item2.includes('byr')) {
        if (!(+item2.split(':')[1] >= 1920 && +item2.split(':')[1] <= 2002)) {
          trulyValid = false;
        }
      }
      if (item2.includes('iyr')) {
        if (!(+item2.split(':')[1] >= 2010 && +item2.split(':')[1] <= 2020)) {
          trulyValid = false;
        }
      }
      if (item2.includes('eyr')) {
        if (!(+item2.split(':')[1] >= 2020 && +item2.split(':')[1] <= 2030)) {
          trulyValid = false;
        }
      }
      if (item2.includes('hgt')) {
        const val = item2.split(':')[1];
        let valid1 = false;
        let valid2 = false;
      ...
      if (item2.includes('hcl')) {
        const val = item2.split(':')[1];
        // eslint-disable-next-line prefer-numeric-literals
        if (!(val.length === 7 && val[0] === '#' && !isNaN(parseInt(val.split('#')[1], 16)))) {
          trulyValid = false;
        }
      }
      if (item2.includes('ecl')) {
        const val = item2.split(':')[1];
        if (
          !(
            val === 'amb' ||
