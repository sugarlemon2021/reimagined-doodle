...
const answer = arr.reduce((acc, item) => {
  const [range, char, str] = item.split(' ');
  const [min, max] = range.split('-');

  // Part 1
  // const occurence = (str.match(new RegExp(char[0], 'g')) || []).length;
  // return occurence >= +min && occurence <= +max ? acc + 1 : acc;

  // Part 2
  const valid = [str[min - 1] === char[0], str[max - 1] === char[0]].filter(Boolean).length === 1;
